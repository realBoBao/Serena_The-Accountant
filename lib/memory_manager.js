...
async function getMemoryDb() {
  const db = await open({ filename: MEMORY_DB_PATH, driver: sqlite3.Database });
...
export async function getArchivedMemory() {
  const db = await getMemoryDb();
  const rows = await db.all(`SELECT * FROM memory_entries WHERE archived = 1 ORDER BY archived_at DESC`);
  await db.close();
  return rows.map((row) => ({ ...row, tags: JSON.parse(row.tags || '[]') }));
}
