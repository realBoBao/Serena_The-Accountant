...
export async function getCachedEmbedding(text) {
  ...
  try {
    ...
    const row = getDbRow(db, 'SELECT embedding, created_at FROM embedding_cache WHERE text_hash = ?', hash);
    if (!row) return null;

    // TTL check
    if (Date.now() - row.created_at > DEFAULT_TTL_MS) {
      runDb(db, 'DELETE FROM embedding_cache WHERE text_hash = ?', hash);
      return null;
    }

    // Update stats
    runDb(db, 'UPDATE embedding_cache SET accessed_at = ?, hit_count = hit_count + 1 WHERE text_hash = ?', Date.now(), hash);
