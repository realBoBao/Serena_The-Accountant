...
async function upsertDocument(collectionName, docId, metadata = {}, chunks = [], embeddings = []) {
  ...
  if (!vectorSize) throw new Error('Cannot infer embedding vector size');
...
async function searchCollection(collectionName, queryEmbedding, topK = 5) {
  ...
  const res = await fetch(`${QDRANT_URL}/collections/${encodeURIComponent(collectionName)}/points/search`, {
    method: 'POST',
    headers: { 'content-type': 'application/json' },
    body: JSON.stringify({ vector: queryEmbedding, limit: topK, with_payload: true }),
  });
...
async function searchAcademic(queryEmbedding, topK = 5) {
  return searchCollection(COLLECTIONS.ACADEMIC, queryEmbedding, topK);
}

async function searchSystem(queryEmbedding, topK = 5) {
  return searchCollection(COLLECTIONS.SYSTEM, queryEmbedding, topK);
}

async function searchDaily(queryEmbedding, topK = 5) {
  return searchCollection(COLLECTIONS.DAILY, queryEmbedding, topK);
}
