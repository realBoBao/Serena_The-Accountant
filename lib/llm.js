 ...
 *
 * Thứ tự fallback:
 *   1. Gemini API        (GEMINI_API_KEY) — model: gemini-2.0-flash (mặc định)
 *   2. OpenRouter        (OPENROUTER_API_KEY) — nhiều model free
 *   3. Local LLM         (llama-server :3001) — Qwen 1.5B
...
function staticFallback(query, context = '') {
...
async function _doAsk(query, { systemPrompt, temperature, maxTokens, opts }) {
  ...
  // ── 1. Gemini (thử lần lượt nhiều model name) ───────────
  if (opts.provider === 'auto' || opts.provider === 'gemini' || !opts.provider) {
    // Xác định danh sách model cần thử
    ...
    for (const modelName of modelsToTry) {
      ...
      try {
        ...
        if (msg.includes('404') || msg.includes('not found') || msg.includes('model')) {
          ...
          break; // Lỗi khác model (auth, network) → dừng thử
        }
      }
    }
  }

  // ── 2. OpenRouter (thử lần lượt các model) ──────────────
...
export default { ask, askWithContext, streamAsk, healthCheck };
