...
async function _getDb(dbPath) {
  ...
  if (_state.DatabaseSync) {
    try {
      ...
      // node:sqlite has prepare() but not run/get/all — patch them
      if (!_state._db.run && _state._db.prepare) {
      ...
      } else if (_state._db.prepare) {
        _state._db.prepare('PRAGMA journal_mode = WAL').run();
        _state._db.prepare('PRAGMA busy_timeout = 5000').run();
      }
    } catch (syncErr) {
      // node:sqlite failed (e.g. Node < 22.5) — fallback to better-sqlite3
      console.warn('[sqlite_adapter] node:sqlite failed, trying better-sqlite3:', syncErr.message);
